...
/*
 *  Constructs a delNeuronDialog as a child of 'parent', with the
 *  name 'name' and widget flags set to 'f'.
 *
 *  The dialog will by default be modeless, unless you set 'modal' to
 *  true to construct a modal dialog.
 */
delNeuronDialog::delNeuronDialog(QWidget* parent, const char* name, bool modal, Qt::WindowFlags fl)
    : QDialog(parent, fl)
{
    setupUi(this);
    setObjectName(name);
 ...
 */
delNeuronDialog::~delNeuronDialog()
{
 ...
 *  language.
 */
void delNeuronDialog::languageChange()
{
    retranslateUi(this);
}

int delNeuronDialog::getNum_IdNeuron(void)
{
    return num_IdNeuron = SpinBox1->cleanText().toInt();
}

bool delNeuronDialog::setNum_IdNeuron( int n_i )
{
    bool ret = false;
    switch(layer) {
    case 0:
        if(n_i > 0 && n_i <= n_in) {
            ...
            SpinBox1->setValue( 1);
        radioLayerInput->setChecked(true) ;
        break;
    case 1:
        ...
        radioLayerHidden->setChecked(true) ;
        break;
    case 2:
        if(n_i > n_in + n_hid && n_i <= n_in + n_hid + n_out) {
            ...
            SpinBox1->setValue( n_in + 1);
        radioLayerOutput->setChecked(true) ;
        break;
    }
...
int delNeuronDialog::getLayer(void)
{
    return layer;
}

void delNeuronDialog::setMinMaxValues( int n_i, int n_h, int n_o )
{
    n_in = n_i;
    n_hid = n_h;
    n_out = n_o;
...
void delNeuronDialog::setLayer( int l_ )
{
    layer = l_;
    switch(layer) {
    case 0:
    ...
    case 1:
        if(n_hid > 0) {
            SpinBox1->setRange(n_in + 1, n_in + n_hid );
            SpinBox1->setValue(n_in + 1);
        }
        break;
    case 2:
        if(n_out > 0) {
            SpinBox1->setRange(n_in + n_hid + 1, n_in + n_hid + n_out );
...
void delNeuronDialog::getText( const QString& texto )
{  
    if(!setNum_IdNeuron(texto.toInt()))
//        QMessageBox::information(nullptr,tr("delNeuron"), tr("Neuron out of range"),tr("&Ok"));
    QMessageBox::information(nullptr,tr("delNeuron"), tr("Neuron out of range"), QMessageBox::Ok);
}


void delNeuronDialog::accept( void )
{
