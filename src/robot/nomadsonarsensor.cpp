//
// C++ Implementation: nomadsonarsensor
//
// Description: 
//
//
...
NomadSonarSensor::NomadSonarSensor(int id, int group_idx, QObject* parent, const char* name) :
                NomadSensor(id, group_idx, parent, name)
{
}

NomadSonarSensor::NomadSonarSensor(int id, int group_idx, double x_, double y_, double rot,
                                   double orient, QObject* parent, const char* name) :
                NomadSensor(id, group_idx, x_, y_, rot, orient, parent, name)
{
}

NomadSonarSensor::NomadSonarSensor(int id, int group_idx, double rad, double rot,
                                   double orient, QObject* parent, const char* name) :
                NomadSensor(id, group_idx, rad, rot, orient, parent, name)
{
}

NomadSonarSensor::NomadSonarSensor(int id, double rad, double start_ang, double end_ang,
                                   int group_idx, QObject* parent, const char* name) :
                NomadSensor(id, rad, start_ang, end_ang, group_idx, parent, name)
{
}

NomadSonarSensor::~NomadSonarSensor()
{
}

int NomadSonarSensor::getRawReading(bool obtain ) 
{
    if(!obtain) return rawReading;

    double rawdata = getConnector()->getStatus()->getSonarData(getIdLocal());
    
    rawReading = rawdata /* inch */ * 25.4 /* mm/inch */;

    return NomadSensor::getRawReading(obtain);
}


void NomadSonarSensor::initMeasure(void)
{
